...
- Директорію `docker/` з необхідною конфігурацією для контейнеризації проєкту за допомогою Docker.
- Файл `docker-compose.yaml` для оркестрації контейнерів, налаштування `volumes` та `networks` (внутрішніх мереж).

### Змінено

- Метод `getImageUrlAttribute` в моделі `Product` - адаптовано формування URL зображень з урахуванням конфігурації Docker-середовища.
