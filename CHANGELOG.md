...
- Файл `0003-laravel-scout-search-system.md` з поясненням чому обрані саме такі рішення щодо системи пошуку товарів.
...
- Оптимізовано вибірку в `category.blade.php`: реалізовано звернення від моделі `Category` напряму до `Product` замість прямого зв'язку через `ProductVariant`.
...
- Файли `0001-cart-system-architecture.md` та `0002-custom-reset-password.md` з поясненням чому обрані саме такі рішення щодо кошика та системи скидання пароля.
